...
{
    /// <summary>
    /// [TEST-ONLY] Minimal fully-deterministic macro agent for cross-engine PARITY testing.
    ///
    /// This is the SIMPLE reference agent: a small hand-written build→gather→train→attack
    /// loop with no RNG and no runtime-specific enumeration order. It is intentionally NOT a
    /// faithful copy of any competitive strategy — for that, see the faithful Det&lt;Name&gt;
    /// snapshots (e.g. DetCommander is a deterministic copy of Commander's full strategy).
    /// This one stays deliberately tiny so it is the cheapest possible "does the engine agree
    /// with itself" baseline. (Renamed from DetCommander 2026-07-12 to free that name for the
    /// faithful full-strategy copy.)
    ///
    /// Every decision is a pure function of the deterministically-ordered game state — no
    /// System.Random / UnityEngine.Random, targets chosen by list order (lowest unitNbr, by
    /// type priority) rather than by float distance (which can tie ambiguously), builds placed
    /// nearest the pawn via DeterministicSort. Running it as BOTH sides produces byte-identical
    /// state in Unity and the headless SimGame. See memory: warcrap-parity-instrumentation.
    /// </summary>
