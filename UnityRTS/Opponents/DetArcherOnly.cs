...
{
    // ============================================================================
    // [DET-SNAPSHOT of ArcherOnly, frozen 2026-07-12] TEST-ONLY deterministic copy.
    //
    // A faithful, verbatim copy of the competitive ArcherOnly agent, included ONLY in the
    // parity test set so cross-engine parity runs on a real, long, engine-exercising
    // game. ArcherOnly was already fully deterministic (no RNG, no Dictionary/HashSet
    ...
    //
    // FROZEN: this is a point-in-time snapshot, NOT maintained in lockstep with ArcherOnly.
    // The competitive ArcherOnly may later change or gain intentional non-determinism; this
    // copy intentionally stays as it was on 2026-07-12. Do not "sync" it. Built to
    // EnemyAgents/PlanningAgent_DetArcherOnly.dll.
    // ============================================================================
    /// <summary>
        ...
        private void DefendTroops(List<int> units, IGameState state, IAgentActions actions)
        {
            foreach (int unitNbr in units)
            {
                ...
                foreach (UnitType ut in new[] { UnitType.WARRIOR, UnitType.ARCHER, UnitType.LANCER,
                                                ...
                                                UnitType.ARCHERY, UnitType.TOWER })
                {
