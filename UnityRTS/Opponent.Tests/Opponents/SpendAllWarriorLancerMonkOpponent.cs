...
{
    /// <summary>
    /// [HARD] Max-spend Warrior + Lancer + Monk: 6 pawns, Barracks + Tower + Monastery.
    /// Trains Warriors from Barracks, Lancers from Tower, and Monks from Monastery
    /// every tick — spending every gold coin as fast as income allows.
    /// Monks heal the most-wounded friendly combat unit below 80% HP
    /// (searches warriors, archers, and lancers).
    /// Attacks with all warriors and lancers when total combat army reaches 6+ combined.
    /// Tests whether warrior/lancer combined pressure with monk sustain overwhelms
    /// opponents under three simultaneous production buildings.
    /// Strategy to beat: archers counter lancers (1.25x); rush before all three
    /// buildings complete (400g + 400g + 350g = 1150g investment).
    /// </summary>
    public class SpendAllWarriorLancerMonkOpponent : PlanningAgentBase
    {
        ...
        public override void Update(IGameState state, IAgentActions actions)
        {
            ...
            // Build order: barracks -> tower -> monastery
            if (myBarracks.Count == 0 && HasBuiltUnit(myBases, state))
                BuildStructure(UnitType.BARRACKS, state, actions);
            else if (myTowers.Count == 0 && HasBuiltUnit(myBarracks, state))
            ...
            // Attack with warriors and lancers when total combat army is large enough
